...
public class AppBarLayoutSpringBehavior extends AppBarLayout.Behavior {
    ...
    private AppBarLayoutScrollBehavior.OnAppBarListener mListener;

    public AppBarLayoutSpringBehavior(AppBarLayoutScrollBehavior.OnAppBarListener listener) {
    ...
    public void setOnAppBarListener(AppBarLayoutScrollBehavior.OnAppBarListener listener) {
        this.mListener = listener;
    }
...
//        }
    ...
    public boolean onLayoutChild(CoordinatorLayout parent, AppBarLayout abl, int layoutDirection) {
        ...
        if(mNormalViewHeight == 0){
            mNormalViewHeight = mAppBarLayout.getHeight();
        }
        ...
        if(mShopInfoLayout == null){
            ...
            Log.d(TAG,"mShopInfoLayout height:"+mShopInfoLayout.getHeight()+" childH:"
                    +mAppBarLayout.getHeight()+" parentH:"+parent.getHeight()+" initHeight:"+initHeight);
            setAppBarLayoutParams(mAppBarLayout, initHeight);
        }

        if (mNormalViewHeight > 0 && mAppBarLayout.getBottom() < mNormalViewHeight) {
            mIsInited = true;
            Log.d(TAG,"end mShopInfoLayout height:"+mShopInfoLayout.getHeight()+" childH:"+mAppBarLayout.getHeight()+" parentH:"+parent.getHeight());

            if (mListener != null) {
                mListener.initEnd(mNormalViewHeight);
            }
            mNormalViewHeight = mAppBarLayout.getHeight();
        }
        return handled;
    ...
    public void onNestedPreScroll(CoordinatorLayout coordinatorLayout, AppBarLayout child,
        ...
        Log.e(TAG, "onNestedPreScroll dy:" + dy + " childBottom " + child.getBottom() + " type:"
                + type+" consumed:"+ Arrays.toString(consumed)+" mOffsetSpring:"+mOffsetSpring
                +" isBottom:"+(child.getBottom() >= mNormalViewHeight)
        +" topBottomOffset:"+getTopBottomOffsetForScrollingSibling()+" getTopAndBottomOffset:"+ getTopAndBottomOffset() + " getDownNestedScrollRange:"+child.getDownNestedScrollRange());
        if(child.getBottom() >= mNormalViewHeight){
            consumed[1] = dy;
            updateSpringOffsetByscroll(coordinatorLayout, child, mOffsetSpring - dy);
        }else{
...
//        super.onNestedPreScroll(coordinatorLayout, child, target, dx, dy, consumed, type);
    ...
    public void onNestedScroll(CoordinatorLayout coordinatorLayout, AppBarLayout child, View target,
        ...
        Log.i(TAG, "onNestedScroll child:" + child.getBottom() + " dyConsumed:" + dyConsumed
                + " dyUnconsumed:" + dyUnconsumed + " type:" + type);
        if (dyUnconsumed < 0) {
//            setHeaderTopBottomOffsetValue(coordinatorLayout, child,
//                    getTopBottomOffsetForScrollingSibling() - dyUnconsumed, -child.getDownNestedScrollRange(), 0, type);
//            setHeaderTopBottomOffsetValue(coordinatorLayout, child, - dyUnconsumed, -child.getDownNestedScrollRange(), 0, type);
        }
    ...
    private void animateFlingSpring(final CoordinatorLayout coordinatorLayout, final AppBarLayout abl, int originNew) {
        ...
        mFlingAnimator.setIntValues(mOffsetSpring, Math.min(mNormalViewHeight * 3 / 2, originNew));
...
//        Log.e("wwq SpringBehavior", "setHeaderTopBottomOffset child:" + appBarLayout.getBottom() + " newOffset:" + newOffset
...
//        return setHeaderTopBottomOffset(coordinatorLayout, appBarLayout, newOffset, minOffset, maxOffset, -1);
    ...
    int setHeaderTopBottomOffsetValue(CoordinatorLayout coordinatorLayout,
                                 AppBarLayout appBarLayout, int newOffset, int minOffset, int maxOffset, int type) {
        int originNew = newOffset;
        final int curOffset = getTopBottomOffsetForScrollingSibling();
        int consumed = 0;
        Log.i(TAG, "setHeaderTopBottomOffset child:" + appBarLayout.getBottom() + " newOffset:" + newOffset
                + " curOffset:" + curOffset + " type:" + type+" mOffsetSpring:"+mOffsetSpring);
    ...
    private int updateSpringByScroll(CoordinatorLayout coordinatorLayout, AppBarLayout appBarLayout, int type, int originNew) {
        int consumed;
        if (appBarLayout.getHeight() >= mNormalViewHeight && type == 1) {
            if (mFlingAnimator == null)
                animateFlingSpring(coordinatorLayout, appBarLayout, originNew);
            return originNew;
        }
        updateSpringOffsetByscroll(coordinatorLayout, appBarLayout, mOffsetSpring + originNew);
//        updateSpringOffsetByscroll(coordinatorLayout, appBarLayout, mOffsetSpring + originNew);
        consumed = getTopBottomOffsetForScrollingSibling() - originNew;
    ...
    private void updateSpringHeaderHeight(CoordinatorLayout coordinatorLayout, AppBarLayout appBarLayout, int offset) {
        if (appBarLayout.getHeight() < mNormalViewHeight || offset < 0) return;
        mOffsetSpring = offset;
        if (mSpringOffsetCallback != null) mSpringOffsetCallback.springCallback(mOffsetSpring);
        setShopInfoSpringChange(mOffsetSpring);
        setAppBarLayoutParams(appBarLayout, mNormalViewHeight + offset);
